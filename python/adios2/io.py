...
class IO:
    ...
    def define_derived_variable(
        ...
        """
        var_impl = None
        print(f"io.py::define_derived_variable()  DerivedVarType = {type}")

        if type is None:
            print(f"Call DefineDerivedVariable({name}, {expression})")
            var_impl = self.impl.DefineDerivedVariable(name, expression)
        else:
            print(f"Call DefineDerivedVariable({name}, {expression}, {type})")
            var_impl = self.impl.DefineDerivedVariable(name, expression, type)
