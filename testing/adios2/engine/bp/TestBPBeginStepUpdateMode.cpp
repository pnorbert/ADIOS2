...
class BPBeginStepUpdateMode : public ::testing::Test
{
protected:
    BPBeginStepUpdateMode() = default;
...
class BPBeginStepUpdateModeReaders
: public BPBeginStepUpdateMode,
...
TEST_P(BPBeginStepUpdateModeReaders, EveryStep)
{
    const ReadMode readMode = GetReadMode();
    std::string fname_prefix =
        "BPBeginStepUpdateMode.EveryStep." + ReadModeToString(readMode);
    ...
    // Write test data using ADIOS2
    {
        ...
        for (int step = 0; step < static_cast<int>(NSteps); ++step)
        {
            ...
            for (int it = 0; it < static_cast<int>(nIterations); ++it)
            {
                ...
                {
                    engine.BeginStep(adios2::StepMode::Update);
                }
...
INSTANTIATE_TEST_SUITE_P(BPBeginStepUpdateMode, BPBeginStepUpdateModeReaders,
