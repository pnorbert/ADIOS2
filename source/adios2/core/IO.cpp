...
#include "adios2/engine/ssc/SscWriter.h"
    ...
    defined(ADIOS2_HAVE_MPI) // external dependencies
...
Engine &IO::Open(const std::string &name, const Mode mode, AMPI_Comm &acomm)
{
    ...
    if (isDefaultEngine || engineTypeLC == "bp3")
    {
        ...
        {
            engine =
                std::make_shared<engine::BP3Reader>(*this, name, mode, acomm);
        }
        else
        {
            engine =
                std::make_shared<engine::BP3Writer>(*this, name, mode, acomm);
        }
    }
    else if (engineTypeLC == "bp4")
    {
        ...
        {
            engine =
                std::make_shared<engine::BP4Reader>(*this, name, mode, acomm);
        }
        else
        {
            engine =
                std::make_shared<engine::BP4Writer>(*this, name, mode, acomm);
        }
    }
    else if (engineTypeLC == "hdfmixer")
    {
        ...
        if (mode == Mode::Read)
            engine =
                std::make_shared<engine::HDF5ReaderP>(*this, name, mode, acomm);
        else
            engine =
                std::make_shared<engine::HDFMixer>(*this, name, mode, acomm);
#else
        throw std::invalid_argument(
    ...
    else if (engineTypeLC == "ssc")
    {
        ...
        if (mode == Mode::Read)
            engine =
                std::make_shared<engine::SscReader>(*this, name, mode, acomm);
        else
            engine =
                std::make_shared<engine::SscWriter>(*this, name, mode, acomm);
#else
        throw std::invalid_argument("ERROR: this version didn't compile with "
    ...
    {
#if defined(ADIOS2_HAVE_SST) && defined(ADIOS2_HAVE_MPI)
        if (mode == Mode::Read)
            engine =
                std::make_shared<engine::SstReader>(*this, name, mode, acomm);
        else
            engine =
                std::make_shared<engine::SstWriter>(*this, name, mode, acomm);
#else
        throw std::invalid_argument("ERROR: this version didn't compile with "
    ...
    else if (engineTypeLC == "hdf5")
    {
        ...
        if (mode == Mode::Read)
            ...
            engine =
                std::make_shared<engine::HDF5WriterP>(*this, name, mode, acomm);
#else
        throw std::invalid_argument("ERROR: this version didn't compile with "
                                    "HDF5 library, can't use HDF5 engine\n");
#endif
    ...
    {
        engine = std::make_shared<engine::NullEngine>(*this, name, mode, acomm);
    }
    else
    {
        if (m_DebugMode)
