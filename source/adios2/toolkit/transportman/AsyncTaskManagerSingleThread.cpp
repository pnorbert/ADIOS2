 ...
 *
 * AsyncTaskManagerSingleThread.cpp
 *
 *  Created on: April 1, 2020
 *      Author: Norbert Podhorszki <pnorbert@ornl.gov>
 */

#include "AsyncTaskManagerSingleThread.h"
...
AsyncTaskManagerSingleThread::AsyncTaskManagerSingleThread()
: AsyncTaskManager()
{
}

AsyncTaskManagerSingleThread::~AsyncTaskManagerSingleThread() { Join(); }

void AsyncTaskManagerSingleThread::SetBufferSize(size_t bufferSizeBytes)
{
    bufferSize = bufferSizeBytes;
}

void AsyncTaskManagerSingleThread::Start()
{
    th = std::thread(&AsyncTaskManagerSingleThread::DrainThread, this);
}

void AsyncTaskManagerSingleThread::Finish()
{
    finishMutex.lock();
    finish = true;
    finishMutex.unlock();
...
void AsyncTaskManagerSingleThread::Join()
{
    if (th.joinable())
    {
 ...
 * calls.
 */
void AsyncTaskManagerSingleThread::DrainThread()
{
    const auto tTotalStart = std::chrono::steady_clock::now();
    Seconds timeTotal = Seconds(0.0);
    ...
    auto lf_Copy = [&](AsyncTaskOperation &fdo, InputFile fdr, OutputFile fdw,
                       size_t count) {
    ...
    while (true)
    {
        ...
        AsyncTaskOperation &fdo = operations.front();
        ...
        case AsyncOp::Copy:
        {
            ...
            ts = std::chrono::steady_clock::now();
            bool append = (fdo.op == AsyncOp::Copy);
