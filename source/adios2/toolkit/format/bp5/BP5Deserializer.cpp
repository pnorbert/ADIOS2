...
void BP5Deserializer::GenerateReadRequest(std::vector<BP5Deserializer::ReadRequest> &Requests,
        ...
        for (size_t Step = StepLoopStart; Step < StepLoopEnd; Step++)
        {
            ...
            for (size_t WriterRank = 0; WriterRank < writerCohortSize; WriterRank++)
            {
                MetaArrayRecOperator *writer_meta_base = (MetaArrayRecOperator *)GetMetadataBase(
                    (struct BP5VarRec *)Req->VarRec, Step, WriterRank);
                ...
                if ((NodeFirstBlock <= Req->BlockID) && (NodeLastBlock >= Req->BlockID))
                {
                    ...
                    if (VarRec->Operator)
                    {
                        ...
                        /* recorded the refactor header info size on upper 16 bits and
                         * the block size of refactored data in lower 48 bits in metadata
                         */
                        std::cout << "### Recorded RR.ReadLength = " << RR.ReadLength << std::endl;
                        RR.OperatorHeaderLength = (RR.ReadLength & 0xFFFF000000000000) >> 48;
                        RR.ReadLength &= 0x0000FFFFFFFFFFFF;
                        std::cout << "### Modified RR.ReadLength = " << RR.ReadLength
                ...
                for (size_t Block = 0; Block < writer_meta_base->BlockCount; Block++)
                {
                                               ...
                                               &intersectionstart[0], &intersectioncount[0]))
                    {
                        ...
                        else if (VarRec->Operator != NULL)
                        {
                             ...
                             * the block size of refactored data in lower 48 bits in metadata
                             */
                            std::cout << "### Recorded RR.ReadLength = " << RR.ReadLength
                                      << std::endl;
                            RR.OperatorHeaderLength = (RR.ReadLength & 0xFFFF000000000000) >> 48;
                            RR.ReadLength &= 0x0000FFFFFFFFFFFF;
                            std::cout << "### Modified RR.ReadLength = " << RR.ReadLength
    ...
    *maxReadSize = 0;
    ...
    try
    {
        for (size_t ReqIndex = 0; ReqIndex < PendingGetRequests.size(); ReqIndex++)
        {
            ...
            GenerateReadRequest(Ret, Req, ReqIndex, doAllocTempBuffers, maxReadSize);
        }
    }
    catch (...)
    {
...
void BP5Deserializer::FinalizeGet(const ReadRequest &Read, const bool freeAddr)
{
    ...
    if (((struct BP5VarRec *)Req.VarRec)->Operator != NULL)
    {
...

        {
            ...
            size_t recordedBlockSize =
                ((MetaArrayRecOperator *)writer_meta_base)->DataBlockSize[Read.BlockID];
            size_t compressedSize = recordedBlockSize & 0x0000FFFFFFFFFFFF;
            std::cout << "### FinalizeGet: recordedBlockSize = " << recordedBlockSize
                      << " CompressedSize = " << compressedSize << std::endl;
            core::Decompress(IncomingData, compressedSize, decompressBuffer.data(), Req.MemSpace,
                             op);
