...
std::vector<int> test_level_1_fi_info()
{
    ...
    std::cout << "Level 1 found " << fabrics_candidates.size()
              << " candidate from " << n << " libfabric interfaces" << std::endl;
    ...
    while (info)
    {
        ...
        if (fabric_candidates[candidatepos] == n)
        {
            std::cout << "Provider: " << info->fabric_attr->prov_name
                      << "    name: " << info->fabric_attr->name
                      << "    domain: " << info->domain_attr->name 
                      << "    ep: " << fi_tostr(&info->ep_attr->type, FI_TYPE_EP_TYPE)
                      << "    protocol: " << fi_tostr(&info->ep_attr->protocol, FI_TYPE_PROTOCOL)
                      << std::endl;
