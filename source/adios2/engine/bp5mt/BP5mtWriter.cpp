...
#include "BP5mtWriter.h"
#include "BP5mtWriter.tcc"
...
BP5mtWriter::BP5mtWriter(IO &io, const std::string &name, const Mode mode,
                         helper::Comm comm)
: Engine("BP5mtWriter", io, name, mode, std::move(comm)), m_BP5Serializer(),
  m_FileDataManager(m_Comm), m_FileMetadataManager(m_Comm),
  m_FileMetadataIndexManager(m_Comm), m_FileMetaMetadataManager(m_Comm),
  m_Profiler(m_Comm)
{
    m_EngineStart = Now();
    PERFSTUBS_SCOPED_TIMER("BP5mtWriter::Open");
    ...
    if (providedMpiMode != MPI_THREAD_MULTIPLE)
    {
        throw std::invalid_argument(
            "Writing " + name +
            " with BP5mt requires that MPI is initialized "
            "with MPI_THREAD_MULTIPLE mode");
    }
    m_Profiler.AddTimerWatch("endstepasync");

    Init();
}

StepStatus BP5mtWriter::BeginStep(StepMode mode, const float timeoutSeconds)
{
    ...
    {
        m_BP5Serializer.InitStep(new MallocV("BP5mtWriter", true,
                                             m_Parameters.InitialBufferSize,
                                             m_Parameters.GrowthFactor));
    }
    else
    {
        m_BP5Serializer.InitStep(new ChunkV("BP5mtWriter",
                                            true /* always copy */,
...
size_t BP5mtWriter::CurrentStep() const { return m_WriterStep; }

void BP5mtWriter::PerformPuts()
{
    PERFSTUBS_SCOPED_TIMER("BP5mtWriter::PerformPuts");
    m_Profiler.Start("PP");
...
void BP5mtWriter::WriteMetaMetadata(
...
uint64_t
BP5mtWriter::WriteMetadata(const std::vector<core::iovec> &MetaDataBlocks,
...
void BP5mtWriter::AsyncWriteDataCleanup()
{
...
void BP5mtWriter::WriteData(format::BufferV *Data)
{
    switch (m_Parameters.AggregationType)
    {
    case (int)AggregationType::EveryoneWritesSerial:
        WriteData_EveryoneWrites(Data);
        break;
    /*case (int)AggregationType::TwoLevelShm:
...
void BP5mtWriter::WriteData_EveryoneWrites(format::BufferV *Data)
{
    ...
    if (a->m_Comm.Rank() > 0)
    {
        a->m_Comm.Recv(&m_DataPos, 1, a->m_Comm.Rank() - 1, 0,
                       "Chain token in BP5mtWriter::WriteData");
    }
    ...
    if (a->m_Comm.Size() > 1)
    {
        ...
        if (a->m_Comm.Rank() == a->m_Comm.Size() - 1)
        {
            a->m_Comm.Isend(&m_DataPos, 1, 0, 0,
                            "Final chain token in BP5mtWriter::WriteData");
        }
        if (a->m_Comm.Rank() == 0)
        {
...
void BP5mtWriter::WriteMetadataFileIndex(uint64_t MetaDataPos,
...
void BP5mtWriter::MarshalAttributes()
{
...
int BP5mtWriter::EndStepAsync(AsyncWriteInfo *info)
{
    PERFSTUBS_SCOPED_TIMER("BP5mtWriter::EndStepAsync");
    m_Profiler.Start("endstepasync");

    auto TSInfo = m_BP5Serializer.CloseTimestep(m_WriterStep, false);
...
void BP5mtWriter::EndStep()
{
    ...
    m_WriteFuture = std::async(std::launch::async, &BP5mtWriter::EndStepAsync,
                               this, m_AsyncWriteInfo);
...
#define declare_type(T)                                                        \
    void BP5mtWriter::DoPut(Variable<T> &variable,                             \
                            typename Variable<T>::Span &span,                  \
                            const bool initialize, const T &value)             \
    {                                                                          \
        PERFSTUBS_SCOPED_TIMER("BP5mtWriter::Put");                            \
...
#define declare_type(T)                                                        \
    void BP5mtWriter::DoPutSync(Variable<T> &variable, const T *data)          \
    {                                                                          \
        PutCommon(variable, data, true);                                       \
    }                                                                          \
    void BP5mtWriter::DoPutDeferred(Variable<T> &variable, const T *data)      \
    {                                                                          \
...
#define declare_type(T, L)                                                     \
    T *BP5mtWriter::DoBufferData_##L(const int bufferIdx,                      \
...
void BP5mtWriter::InitParameters()
{
...
void BP5mtWriter::InitAggregator()
{
    // in BP5mt, aggregation is "always on", but processes may be alone, so
...
void BP5mtWriter::InitTransports()
{
...
void BP5mtWriter::MakeHeader(format::BufferSTL &b, const std::string fileType,
                             const bool isActive)
{
...
void BP5mtWriter::NotifyEngineAttribute(std::string name,
                                        DataType type) noexcept
{
    m_MarshaledAttributesCount = 0;
}

void BP5mtWriter::EnterComputationBlock() noexcept
{
...
void BP5mtWriter::ExitComputationBlock() noexcept
{
    if (m_InComputationBlock)
    {
...
void BP5mtWriter::FlushData(const bool isFinal)
{
    BufferV *DataBuf;
    if (m_Parameters.BufferVType == (int)BufferVType::MallocVType)
    {
        DataBuf = m_BP5Serializer.ReinitStepData(
            new MallocV("BP5mtWriter", false, m_Parameters.InitialBufferSize,
                        m_Parameters.GrowthFactor));
    }
    else
    {
        DataBuf = m_BP5Serializer.ReinitStepData(
            new ChunkV("BP5mtWriter", false /* always copy */,
                       m_Parameters.BufferChunkSize));
    }
...
void BP5mtWriter::Flush(const int transportIndex) { FlushData(false); }

void BP5mtWriter::DoClose(const int transportIndex)
{
    PERFSTUBS_SCOPED_TIMER("BP5mtWriter::Close");
...
void BP5mtWriter::FlushProfiler()
{
...
void BP5mtWriter::PopulateMetadataIndexFileContent(
...
{
    PERFSTUBS_SCOPED_TIMER("BP5mtWriter::PopulateMetadataIndexFileContent");
    auto &buffer = b.m_Buffer;
    auto &position = b.m_Position;
